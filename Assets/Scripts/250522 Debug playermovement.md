...
_Generado el Thu May 22 15:13:50 EST 2025_\n
...
public class UnitGroundFollower : MonoBehaviour
{
    ...
    {
        // Use a slightly larger radius for detection
        Collider[] hits = Physics.OverlapSphere(transform.position, HexRenderer.SharedOuterRadius, LayerMask.GetMask(terrainLayer));
...
public class UnitHighlightFollower : MonoBehaviour
{
    ...
    private GameObject CreateHighlightObject(string name, Material mat, Color color)
    {
        ...
        // MeshCollider mc = highlight.AddComponent<MeshCollider>(); // No need for collider on highlight itself
        ...
        float halfRadius = HexRenderer.SharedOuterRadius * 0.95f; // Slightly smaller to avoid Z-fighting
        ...
        // mc.sharedMesh = highlightMesh; // Not needed as we removed collider
        // mc.convex = false;
...
public class UnitMover : MonoBehaviour
{
    ...
    public void MoveTo(HexBehavior targetHex)
    {
        ...
        {
            Debug.LogWarning("UnitMover: Current hex or target hex is null. Cannot move.");
            return;
        }
    ...
    private IEnumerator FollowPath(Queue<HexBehavior> path)
    {
        ...
        while (path.Count > 0)
        {
            ...
            while (Vector3.Distance(transform.position, targetPosition) > minHeightThreshold) // Use minHeightThreshold as a small distance epsilon
            {
    ...
    {
        Collider[] colliders = Physics.OverlapSphere(transform.position, HexRenderer.SharedOuterRadius, LayerMask.GetMask(terrainLayer));
        foreach (var col in colliders)
        {
    ...
    private void SnapToGroundAtStart()
    {
        ...
        {
            Debug.LogWarning($"UnitMover: Could not find hex to snap to at Start for {gameObject.name}.");
        }
    }
