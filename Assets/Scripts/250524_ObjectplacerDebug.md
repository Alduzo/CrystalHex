...
_Generado el Fri May 23 02:06:16 EST 2025_\n
...
private static IEnumerator DelayedPlaceFeature(HexBehavior hex, GameObject prefab)
{
    ...
    GameObject placed = HexObjectPlacer.PlaceOnHex(hex, prefab);
if (placed != null)
{
    AutoPlaceOnTerrain placer = placed.GetComponent<AutoPlaceOnTerrain>();
    if (placer != null)
    {
        placer.TryPlace(); // coloca al objeto en tiempo controlado
    }
}
}
...
public class AutoPlaceOnTerrain : MonoBehaviour
{
    ...
    // New: Adjustable radius for terrain detection
    ...
    private IEnumerator Start()
    {
        ...
        Debug.LogWarning($"⚠️ {gameObject.name} no pudo colocarse sobre el terreno tras {maxAttempts} intentos."); // Line 30
    }

    private bool TryPlaceOnTerrain()
    {
        // Use the new configurable radius for detection
        ...
        Debug.Log($"🔎 AutoPlace buscando colliders para {gameObject.name}: {colliders.Length}");
        foreach (var col in colliders)
        {
            Debug.Log($" - 🎯 Hit: {col.name} en layer {LayerMask.LayerToName(col.gameObject.layer)}");
            HexRenderer hex = col.GetComponentInParent<HexRenderer>();
            if (hex != null && transform.childCount > 0)
            {
                Transform visual = GetVisualChild(transform);
if (visual == null)
{
    Debug.LogWarning($"❌ {name} no tiene Renderer hijo visible. No se puede alinear.");
    continue;
}
    ...
    Transform GetVisualChild(Transform parent)
    {
    ...
    private bool IsAlreadyPlaced()
{
    ...
    foreach (var col in colliders)
    {
        ...
        if (hex != null)
        {
            return true; // Ya está sobre un hex
        }
    }
    return false;
}
...
public class PlayerPlacementHelper : MonoBehaviour
{
    ...
    public bool TryPlace()
    {
        ...
        Collider[] colliders = Physics.OverlapSphere(transform.position, placementDetectionRadius, LayerMask.GetMask(terrainLayerName));
        Debug.Log($"🔎 PlayerPlacement buscando colliders: {colliders.Length}");
        foreach (var col in colliders)
        {
            Debug.Log($" - 🎯 Hit: {col.name} en layer {LayerMask.LayerToName(col.gameObject.layer)}");
...
return true;
        ...
        Debug.LogWarning($"⚠️ {gameObject.name} no pudo colocarse sobre terreno usando VisualTop..."); // Line 52
        return false;
    }
...
private Transform GetVisualChild(Transform parent)
{
    Renderer[] renderers = parent.GetComponentsInChildren<Renderer>();
    if (renderers.Length == 0) return null;
    return renderers[0].transform;
