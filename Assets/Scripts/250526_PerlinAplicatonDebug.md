...
_Generado el Mon May 26 15:08:00 EST 2025_\n
   ...
   public void UpdateChunks(Vector2Int playerChunkCoord)
{
    ...
    if (chunksToKeep.Count > 0) // 🔥 Sincronizar vecinos incluso en arranque
    {
        ...
        foreach (var coord in chunksToKeep)
        {
            if (loadedChunks.TryGetValue(coord, out var chunk))
            {
                ...
                // 🚨 Sincronización global por cada chunk
...
public class WorldMapManager : MonoBehaviour
{
    ...
    [SerializeField] private float mountainMaxElevation = 200f;
    ...
    // MÉTODO CENTRAL PARA INICIALIZAR EL MUNDO
private void Start()
{
    Debug.Log("🌍 Generando mundo con PerlinSettings actualizado. Seed: " + perlinSettings.seed);
    ResetWorld();
    ChunkManager.Instance?.UpdateChunks(Vector2Int.zero);
}


    // RECARGA DE PERLINSETTINGS ACTUALIZADO DESDE EL EDITOR
    public void LoadPerlinSettingsFromEditor()
    {
        var editor = GameObject.FindFirstObjectByType<PerlinSettingsController>();
        if (editor != null && editor.perlinSettings != null)
        {
            perlinSettings = editor.perlinSettings;
            Debug.Log("📥 PerlinSettings actualizado desde el editor.");
        }
        else
        {
            Debug.LogWarning("⚠️ No se encontró PerlinSettingsController o PerlinSettings.");
        }
    }

    // RESETEO COMPLETO DEL MUNDO Y CHUNKS
    public void ResetWorld()
    {
        ...
        Debug.Log("🌎 Mundo reseteado.");
    }

    // --- Resto del código (sin cambios) ---
    // Incluye GetOrGenerateHex, DetermineTerrainType, GetAllHexes, CalculateElevation, etc.
    // Copiado desde tu script original, manteniendo la lógica funcional.
